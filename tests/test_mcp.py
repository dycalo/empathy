...
def test_load_mcp_from_global(tmp_path: Path) -> None:
    ...
    config = {"mcpServers": {"test": {"command": "node", "args": ["-v"], "env": {"FOO": "bar"}}}}
...
def test_load_mcp_returns_only_enabled(tmp_path: Path) -> None:
    ...
    config = {"mcpServers": {"test": {"command": "node"}, "other": {"command": "python"}}}
    (tier_dir / "mcp.json").write_text(json.dumps(config))
...
def test_load_mcp_overrides(tmp_path: Path) -> None:
    ...
    (global_dir / "therapist" / "mcp.json").write_text(
        json.dumps({"mcpServers": {"test": {"command": "node"}}})
    )

    (dialogue_dir / "therapist" / "mcp.json").write_text(
        json.dumps({"mcpServers": {"test": {"command": "python", "args": ["-m"]}}})
    )

    provider = load_mcp_provider(
        "therapist", global_dir=global_dir, dialogue_dir=dialogue_dir, enabled_mcp_servers=["test"]
    )
